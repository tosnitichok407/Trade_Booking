...
function usePriceFeed(symbol, type) {
...
function AssetPanel({ symbol, type, capital, onTrade }) {
  const { price, loading } = usePriceFeed(symbol, type);
  ...
  useEffect(() => {
    ...
    pricesRef.current = [...pricesRef.current.slice(-59), price];
    setSpark([...pricesRef.current.slice(-30)]);
    const sig = generateSignal(pricesRef.current);
    setSignal(sig);

    // Auto execute
    const riskAmt  = capital * 0.02;                      // 2% risk per trade
    const stopDist = price * 0.01;                        // 1% stop distance
    ...
    if (sig === "BUY" && !position) {
      ...
      const tradePnl = +((price - position.entry) * position.qty).toFixed(2);
      setPnl(p => +(p + tradePnl).toFixed(2));
      setTrades(t => t.map((tr, i) => i === 0 ? { ...tr, exit: price, pnl: tradePnl } : tr));
      setPosition(null);
      onTrade?.({ symbol, side: "CLOSE", qty: position.qty, entry: position.entry, exit: price, pnl: tradePnl });
    }
  }, [price]);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {[
          { label: "EMA9",  val: calcEMA(pricesRef.current, 9) },
          { label: "EMA21", val: calcEMA(pricesRef.current, 21) },
          { label: "RSI",   val: calcRSI(pricesRef.current, 14) },
        ].map(({ label, val }) => (
          <div key={label} style={{
